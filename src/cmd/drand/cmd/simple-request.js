...
const checkAggregateSig = async (round, address) => {
  ...
  try {
    const expectedResult = await cosmos.get(
      `/cosmos/tx/v1beta1/txs?events=wasm.function_type%3D%27aggregate_sig%27&events=wasm.round%3D%27${round}%27&events=wasm.contract_address%3D%27${address}%27`
    );
